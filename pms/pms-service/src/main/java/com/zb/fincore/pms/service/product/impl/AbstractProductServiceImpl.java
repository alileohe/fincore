...
import com.zb.fincore.ams.facade.dto.req.QueryProductRelatedAssetInfoRequest;
...
import com.zb.fincore.common.topic.TopicConstants;
...
import com.zb.fincore.pms.common.enums.ProductSyncStatusEnum;
...
import com.zb.fincore.pms.facade.product.dto.req.WaitingEstablishRequest;
...
import com.zb.fincore.pms.service.ProducerService;
...
import com.zb.fincore.ta.ladder.facade.dto.ProductAssetDTO;
import com.zb.fincore.ta.ladder.facade.dto.ProductDTO;
import com.zb.fincore.ta.ladder.facade.dto.ProductLadderDTO;
import com.zb.fincore.trade.ladder.facade.dto.ProductReceiveNotifyDTO;
...
public abstract class AbstractProductServiceImpl implements BaseProductService {
    ...
    @Override
    public BaseResponse putProductOnLine(@Valid UpdateProductSaleStatusRequest req) {
    ...
    @Transactional(propagation = Propagation.REQUIRED, rollbackFor = {Exception.class})
    @Override
    public BaseResponse putProductOnLineForP2P(@Valid UpdateProductSaleStatusRequest req) {
        ...
        // 如果同一资产池，同一起息日，同一产品期限，并且产品销售状态为上线的产品已存在，不能上线
        Map<String, Object> tempParams = new HashMap<String, Object>();
        tempParams.put("assetPoolCode", product.getAssetPoolCode());// 资产池编号
        tempParams.put("valueTime", product.getProductPeriod().getValueTime());// 起息时间
        tempParams.put("investPeriod", product.getProductPeriod().getInvestPeriod());// 投资期限
        tempParams.put("saleChannelCode", ChannelEnum.ZD.getCode());// 销售渠道
        tempParams.put("saleStatus", ProductSaleStatusEnum.PRODUCT_SALE_STATUS_ON_LINE.getCode());// 销售状态
		List<Product> productList = productDao.queryProductListByCondition(tempParams, new Page());
		if (productList != null && productList.size() > 0) {
			return BaseResponse.build(Constants.PRODUCT_SAME_ASSETPOOL_VALUETIME_INVESTPERIOD_ONLINEEXIST_CODE,
    ...
    public RegisterProductResponse registerProductBaseInfo(@Valid RegisterProductBaseInfoRequest req) throws Exception {
        ...
        //2: 按照规则生成产品编号
        String productCode = sequenceService.generateBusinessCode(req.getPatternCode(), Constants.SEQUENCE_NAME_PREFIX_PRODUCT, 4);
    ...
    public RegisterProductResponse registerProductProfitInfo(@Valid RegisterProductProfitInfoRequest req) throws Exception {
        ...
        RegisterProductResponse resp = validator.checkRegisterProductProfitInfoParameter(req);
        if (null != resp) {
            return resp;
        }
    ...
    public PageQueryResponse<ProductModel> queryProductListForP2P(@Valid QueryProductListRequest req) throws Exception {
        //req.setCollectStatus(ProductCollectStatusEnum.PRODUCT_COLLECT_STATUS_COLLECTING.getCode());
        req.setSaleChannelCode(ChannelEnum.ZD.getCode());
        req.setSaleStatus(ProductSaleStatusEnum.PRODUCT_SALE_STATUS_ON_LINE.getCode());
        req.setSyncStatus(ProductSyncStatusEnum.UN_SYNC.getCode());
        req.setSaleEndTime(new Date());
        return queryProductList(req);
    ...
    public PageQueryResponse<ProductModel> queryProductList(QueryProductListRequest req) throws Exception {
        ...
        params.put("patternCode", req.getPatternCode());//形态代码
...
//                productModel.setProductApprovalModel(productApprovalModel);
    ...
    public BaseResponse putProductValuing() throws Exception {
        ...
        saleStatusList.add(ProductSaleStatusEnum.PRODUCT_SALE_STATUS_ON_LINE.getCode());
        saleStatusList.add(ProductSaleStatusEnum.PRODUCT_SALE_STATUS_OFF_LINE.getCode());
        params.put("saleChannelCode", ChannelEnum.ZD.getCode());
        params.put("saleStatusList", saleStatusList);
        params.put("approvalStatus", ProductApprovalStatusEnum.APPROVAL_SUCCESS.getCode());//审核通过
        params.put("collectStatus", ProductCollectStatusEnum.PRODUCT_COLLECT_STATUS_COLLECTING.getCode());//募集期
        params.put("querySaleTimeOutProduct", Constants.CONSTANT_VALUE_1);//查询募集期结束产品列表
    ...
    /**
     * 募集期结束 --》产品存续期    JOB调用 实际处理逻辑
     *
     * @return
     * @throws Exception
     */
    @Transactional(propagation = Propagation.REQUIRED, rollbackFor = {Exception.class})
    private void doProductValuing(Product p) throws Exception {
        Map<String, Object> params = new HashMap<String, Object>();
        //更新产品募集状态为 存续期
        params.put("id", p.getId());
        params.put("originalStatus", p.getCollectStatus());
