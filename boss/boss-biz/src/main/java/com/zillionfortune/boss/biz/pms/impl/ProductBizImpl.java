...
public class ProductBizImpl implements ProductBiz {
    ...
    private String intervalProductInfoQueryUrl;
    ...
    public BaseWebResponse registerProduct(RegisterProductRequest req, String createBy, Integer userId) {
        ...
        try {
            ...
            // 调用产品注册http服务接口
            RegisterProductResponse registerProductResponse = productRegisterUseHttp(req);
    ...
    public BaseWebResponse putProductOffLine(UpdateProductSaleStatusRequest req) {
        ...
        try {
            ...
            // 调用产品下线http服务接口
            BaseResponse baseResponse = putProductOffLineUseHttp(req);
    ...
    public BaseWebResponse countUpProductStatus() {
        ...
        try {
            ...
            // 调用产品列表查询http服务接口
            ProductStatisticsResponse response = productStatistics(req);
...
//            }
    ...
    public BaseWebResponse queryProductApprovalList(QueryProductApprovalListRequest req, String createBy) {
        ...
        try {
            ...
            QueryProductApprovalInfoResponse productApprovalInfoResponse = queryProductApprovalListUseHttp(req);
    ...
    private void setQueryProductDetailModel(QueryProductDetailModel model, QueryProductInfoResponse resp) {
        ...
        String reservationTotalAmount = "";
        if ( productModel.getReservationTotalAmount() != null) {
        	reservationTotalAmount = productModel.getReservationTotalAmount().toString();
        }
        model.setReservationTotalAmount(reservationTotalAmount); // 预约募集金额）
        String actualTotalAmount = "";
        if ( productModel.getActualTotalAmount() != null) {
        	actualTotalAmount = productModel.getActualTotalAmount().toString();
        }
        model.setActualTotalAmount(actualTotalAmount); // 已募集金额
     ...
     * @throws Exception
     */
    public RegisterProductResponse productRegisterUseHttp(RegisterProductRequest req) throws Exception {
        RegisterProductResponse registerProductResponse = null;
        String respContent = null;
        // 调用远程服务
        log.debug("产品注册 调用pms请求参数：" + JSONObject.toJSONString(req));
        respContent = aesHttpClientUtils.sendPostRequest(productRegisterUrl, JSONObject.toJSONString(req));
        log.debug("产品注册 调用pms响应参数：：" + respContent);
        // 将json字符创转换成json对象
    ...
    public RegisterProductResponse internalProductRegisterUseHttp(RegisterProductRequest req) throws Exception {
        ...
        // 调用远程服务
        log.debug("产品注册 调用pms请求参数：" + JSONObject.toJSONString(req));
        respContent = aesHttpClientUtils.sendPostRequest(internalProductRegisterUrl, JSONObject.toJSONString(req));
     ...
     * @throws Exception
     */
    public PageQueryResponse<ProductModel> productListQueryUseHttp(QueryProductListRequest req) throws Exception {
        PageQueryResponse<ProductModel> productModelPageQueryResponse = null;
        String respContent = null;
        // 调用远程服务
        log.debug("产品列表查询 调用pms请求参数：" + JSONObject.toJSONString(req));
        respContent = aesHttpClientUtils.sendPostRequest(productListQueryUrl, JSONObject.toJSONString(req));
        log.debug("产品列表查询 调用pms响应参数：：" + respContent);
    ...
    public QueryProductInfoResponse productInfoQueryUseHttp(QueryProductInfoRequest req,String productType) throws Exception {
        ...
        if(String.valueOf(ProductTypeEnum.INTERVAL.code()).equals(productType)){
        	productInfoQueryUrl=intervalProductInfoQueryUrl;
        }
        
        String respContent = null;
        // 调用远程服务
        log.debug("产品详情查询 调用pms请求参数：" + JSONObject.toJSONString(req));
        respContent = aesHttpClientUtils.sendPostRequest(productInfoQueryUrl, JSONObject.toJSONString(req));
        log.debug("产品详情查询 调用pms响应参数：：" + respContent);
        // 将json字符创转换成json对象
        if (StringUtils.isNotBlank(respContent)) {
    ...
    public BaseResponse putProductOnLineUseHttp(UpdateProductSaleStatusRequest req,String productType) throws Exception {
        ...
        if(String.valueOf(ProductTypeEnum.INTERVAL.code()).equals(productType)){
        	productOnLineUrl=intervalProductOnLineUrl;
        }
        String respContent = null;
        // 调用远程服务
        log.debug("产品上线 调用pms请求参数：" + JSONObject.toJSONString(req));
        respContent = aesHttpClientUtils.sendPostRequest(productOnLineUrl, JSONObject.toJSONString(req));
        log.debug("产品上线 调用pms响应参数：：" + respContent);
        // 将json字符创转换成json对象
        if (StringUtils.isNotBlank(respContent)) {
    ...
    private BaseResponse updateProductCollectAmountUseHttp(UpdateProductCollectAmountRequest req) throws Exception {
        ...
        // 调用远程服务
        log.debug("产品上线 调用pms请求参数：" + JSONObject.toJSONString(req));
        respContent = aesHttpClientUtils.sendPostRequest(productCollectAmountUrl, JSONObject.toJSONString(req));
        log.debug("产品上线 调用pms响应参数：：" + respContent);
        // 将json字符创转换成json对象
        if (StringUtils.isNotBlank(respContent)) {
            baseResponse = JSONObject.parseObject(respContent, BaseResponse.class);
     ...
     * @throws Exception
     */
    public BaseResponse putProductOffLineUseHttp(UpdateProductSaleStatusRequest req) throws Exception {
        BaseResponse baseResponse = null;
        String respContent = null;
        // 调用远程服务
        log.debug("产品下线 调用pms请求参数：" + JSONObject.toJSONString(req));
        respContent = aesHttpClientUtils.sendPostRequest(productOffLineUrl, JSONObject.toJSONString(req));
        log.debug("产品下线 调用pms响应参数：：" + respContent);
        // 将json字符创转换成json对象
        if (StringUtils.isNotBlank(respContent)) {
            baseResponse = JSONObject.parseObject(respContent, BaseResponse.class);
        }
    ...
    public BaseResponse productApprovalUseHttp(ApproveProductRequest req,String productType) throws Exception {
        ...
        String respContent = null;
        if(String.valueOf(ProductTypeEnum.INTERVAL.code()).equals(productType)){
        	productApprovalUrl=internalProductApprovalUrl;
        }
        // 调用远程服务
        log.debug("产品审核 调用pms请求参数：" + JSONObject.toJSONString(req));
		respContent = aesHttpClientUtils.sendPostRequest(productApprovalUrl, JSONObject.toJSONString(req));
        log.debug("产品审核 调用pms响应参数：：" + respContent);
        // 将json字符创转换成json对象
        if (StringUtils.isNotBlank(respContent)) {
            baseResponse = JSONObject.parseObject(respContent, BaseResponse.class);
        }
     ...
     * @throws Exception
     */
    public QueryProductApprovalInfoResponse queryProductApprovalListUseHttp(QueryProductApprovalListRequest req) throws Exception {
        QueryProductApprovalInfoResponse productApprovalInfoResponse = null;
        String respContent = null;
        // 调用远程服务
        log.debug("产品审核信息列表查询 调用pms请求参数：" + JSONObject.toJSONString(req));
        respContent = aesHttpClientUtils.sendPostRequest(productApprovalInfoListUrl, JSONObject.toJSONString(req));
        log.debug("产品审核信息列表查询 调用pms响应参数：：" + respContent);
    ...
    public ProductStatisticsResponse productStatistics(QueryProductListRequest req) throws Exception {
        ...
        String respContent = null;
        // 调用远程服务
        log.debug("产品统计查询 调用pms请求参数：" + JSONObject.toJSONString(req));
        respContent = aesHttpClientUtils.sendPostRequest(productStatisticsUrl, JSONObject.toJSONString(req));
        log.debug("产品统计查询 调用pms响应参数：：" + respContent);
        // 将json字符创转换成json对象
        if (StringUtils.isNotBlank(respContent)) {
