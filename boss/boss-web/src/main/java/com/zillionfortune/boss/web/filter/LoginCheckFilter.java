	...
	public void doFilter(ServletRequest request, ServletResponse response,
		...
		try {
			...
			if(StringUtils.isNotBlank(reqUrl) 
					&& 
					(reqUrl.indexOf("operationLogin.html")>0
						|| reqUrl.indexOf("download.html")>0)){
				chain.doFilter(request, response);
				return;
			}
			...
			if(sessionId==null || StringUtils.isBlank(sessionId) ){
				log.info("------------校验是否登录--------------doFilter方法--sessionID为空");
				System.out.println("------------校验是否登录--------------doFilter方法--sessionID为空");
				webResponse = new BaseWebResponse(RespCode.SUCCESS.code(), 
						ResultCode.USER_NOT_LOGIN.code(), ResultCode.USER_NOT_LOGIN.desc());
				...
				if(userRedisService.check(userId)==false ){//userInfo信息不存在
					log.info("------------校验userInfo--------------doFilter方法--userInfo不存在");
					System.out.println("------------校验userInfo--------------doFilter方法--userInfo不存在");
					webResponse = new BaseWebResponse(RespCode.SUCCESS.code(), 
							ResultCode.USER_NOT_LOGIN.code(), ResultCode.USER_NOT_LOGIN.desc());
					resp.getWriter().print(JSONObject.toJSON(webResponse));
					return;
