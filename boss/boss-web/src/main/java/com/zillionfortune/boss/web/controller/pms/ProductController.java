...
public class ProductController {
	...
	public BaseWebResponse registerProduct(@RequestBody ProductRegisterRequestVo vo) {
		...
		try {
			...
			// step3: 调用产品注册
			resp = productBiz.registerProduct(req, createBy, userId);
		} catch (Exception e) {
	...
	public BaseWebResponse queryProductList(@RequestBody QueryProductListVo vo) {
		...
		try {
			...
			// step3: 调用产品列表查询
			resp = productBiz.queryProductList(req);
		} catch (Exception e) {
	...
	public BaseWebResponse putProductOffLine(@RequestBody PutOrOutProductOffLineRequestVo vo) {
		...
		try {
			...
			// step3: 调用产品下线
			resp = productBiz.putProductOffLine(req);
		} catch (Exception e) {
	...
	public BaseWebResponse countUpProductStatus() {
		...
		try {
			// 调用产品状态分类统计
			resp = productBiz.countUpProductStatus();
		} catch (Exception e) {
	...
	public BaseWebResponse queryProductApprovalList(@RequestBody QueryProductApprovalListVo vo) {
		...
		try {
			...
			// step3: 调用产品审核信息列表查询
			resp = productBiz.queryProductApprovalList(req, httpSessionUtils.getCuruser().getName());
		} catch (Exception e) {
	...
	private void copyRequestForProductRegister(ProductRegisterRequestVo vo, RegisterProductRequest req) {
		...
		if (StringUtils.isNotBlank(vo.getExpireTime())) {
			try {
				req.setExpectExpireTime(format.parse(vo.getExpireTime()));  // 到期日（预期）
			} catch (ParseException e) {
				e.printStackTrace();
			} 
		}
