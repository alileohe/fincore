...
//			op.setModifyBy(CommonConstants.TRADE_OPERATE_SYSTEM);
	...
	public OperationLoginResponse operationLogin(OperationLoginRequest req) {
		...
		try {
			...
			BeanUtilsWrapper.copyProperties(op,req);
			op.setPassword(PassUtil.getPassword(op.getPassword()));
			List<User> userList = opUserDao.queryList(op);
			if(userList==null || userList.isEmpty()){
				...
				for(User opUser : userList){
					if(opUser.getDeleteFlag()==DeleteFlag.EXISTS.code()){
						
//						//将Token存到rdis中
//						TokenModel tokenModel = tokenManager.createToken(opUser.getId()+"");
						// 获取用户拥有的授权级别
	...
	@Override
	public OperationUserMenuResponse operationUserMenu(
			OperationUserMenuRequest req) {
	...
	private List<Menu> getChild(Menu omt,List<Menu> list){
		...
		for(Menu menu:list){
			...
			if(menu.getParentId() != null 
					&& omt.getId() != null
					&& menu.getParentId().intValue()==omt.getId().intValue()){
				childList.add(menu);
			}
		}
	...
	private List<String> getSignLevels(Integer userId) {
		...
		if (roles != null && roles.size() > 0) {
			for (Role tempRole : roles) {
				if (StringUtils.isNotBlank(tempRole.getSignLevel())
						&& !signLevels.contains(tempRole.getSignLevel())) {
