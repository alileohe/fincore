...
public class OssFileUtil {
     ...
     * 阿里云ACCESS_ID
     */
    public static final String ACCESS_KEY_ID = PropertiesUtil.getValue("ACCESS_KEY_ID");

    /**
     * 阿里云ACCESS_KEY
     */
    public static final String ACCESS_KEY_SECRET = PropertiesUtil.getValue("ACCESS_KEY_SECRET");

    /**
     * 阿里云OSS_ENDPOINT Url
     */
    public static final String OSS_ENDPOINT = PropertiesUtil.getValue("OSS_ENDPOINT");

    /**
     * 阿里云OSS_ENDPOINT Url
     */
    public static final String OSS_ENDPOINT_URL = PropertiesUtil.getValue("OSS_ENDPOINT_NAME");
    /**
     * 阿里云BUCKET_NAME OSS
     */
    public static final String BUCKET_NAME = PropertiesUtil.getValue("BUCKET_NAME");

    /**
     * 是否是调试模式
     */
    public static final boolean IS_DEBUG_MODE = PropertiesUtil.getValue("IS_DEBUG_MODE").toUpperCase().equals("Y");

    /**
     * 调试模式文件存储目录路径
     */
    public static final String DEBUG_FILE_STORAGE_PATH = PropertiesUtil.getValue("DEBUG_FILE_STORAGE_PATH");
    ...
    public static boolean checkPointDownload(String fileName, String storageFullName) throws Throwable {
        // 下载请求，10个任务并发下载，启动断点续传
        DownloadFileRequest downloadFileRequest = new DownloadFileRequest(BUCKET_NAME, fileName);
        downloadFileRequest.setDownloadFile(storageFullName);
        downloadFileRequest.setTaskNum(10);
        downloadFileRequest.setEnableCheckpoint(true);
        // 下载文件
        DownloadFileResult downloadRes = client.downloadFile(downloadFileRequest);
        // 下载成功时，会返回文件的元信息
        return (downloadRes.getObjectMetadata() != null);
    }

    public static boolean download(String fileName, String storageFullName) throws Throwable {
        return (client.getObject(new GetObjectRequest(BUCKET_NAME, fileName), new File(storageFullName)) != null);
    }

    public static void downloadOssFile(String fileName, String storageFullName) {
        LOGGER.info("下载OSS文件{}至本地路径{}", fileName, storageFullName);
        for (int i = 0; i < 120; i++) {
            try {
                if (checkPointDownload(fileName, storageFullName)) {
                    LOGGER.info("下载OSS文件{}至本地路径{}成功", fileName, storageFullName);
                    break;
                } else {
                    Thread.sleep(500);
                }
            } catch (Throwable e) {
                LOGGER.error("下载OSS文件异常", e);
            }
        }
    }

    public static void downloadOssFile(String fileName) {
        String storageFullName = FilenameUtils.concat(DEBUG_FILE_STORAGE_PATH, fileName);
        if (IS_DEBUG_MODE) {
            LOGGER.info("使用DEBUG_MODE,跳过下载,将使用本地文件{}", storageFullName);
        } else {
            File storageFile = new File(storageFullName);
            if (storageFile.exists()) {
                LOGGER.info("本地文件{}已经存在", storageFullName);
            } else {
                downloadOssFile(fileName, storageFullName);
            }
        }
    }

    public static InputStream downloadAndReadFile(String fileName) {
        String storageFullName = FilenameUtils.concat(DEBUG_FILE_STORAGE_PATH, fileName);
        if (IS_DEBUG_MODE) {
            LOGGER.info("使用DEBUG_MODE,跳过下载,将使用本地文件{}", storageFullName);
        } else {
        ...
        File file = new File(storageFullName);
        if (!file.exists()) {
            LOGGER.error("本地文件{}不存在,无法读取文件流", storageFullName);
            return null;
        } else {
            try {
                FileInputStream is = new FileInputStream(file);
    ...
    public static void deleteFile(String fileName) {
        String storageFullName = FilenameUtils.concat(DEBUG_FILE_STORAGE_PATH, fileName);
        if (IS_DEBUG_MODE) {
            LOGGER.info("使用DEBUG_MODE,跳过删除文件{}", storageFullName);
        } else {
            LOGGER.info("删除本地文件{}", storageFullName);
            File file = new File(storageFullName);
            if (file.exists()) {
                for (int i = 0; i < 120; i++) {
                    try {
    ...
    public static void main(String[] args) {
        ObjectMetadata objectMeta = new ObjectMetadata();//元数据对象
        String fileName = "201_0417070083_20170722134848813.txt";
        File file = new File("D:\\" + fileName);
        try {
            OSSClient client = new OSSClient("http://oss-cn-shanghai.aliyuncs.com/", "LTAIgpkYIYkjmJ31", "QNfNCMwbNScmevVu8E8YHJrwSYloMF");
            FileInputStream input = new FileInputStream(file);
            objectMeta.setContentLength(Long.parseLong(input.available() + ""));//设置传输文件长度
            objectMeta.setContentType(readFileType(fileName.substring(fileName.lastIndexOf("."))));//设置文件类型
            PutObjectResult result = client.putObject("bucket-zb", fileName, input, objectMeta);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
